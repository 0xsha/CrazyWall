<template>
  <div class="text-gray-400">
    ...
    <p v-else-if="$fetchState.error">Error while fetching ip detail please retry later</p>
    <div v-else>


      <div class="flex items-center justify-center bg-draculaBackground py-4 mt-2 mb-2 rounded-lg">

        <img class="h-14 w-14 mr-2" src="~/assets/icons/goal.svg" alt="">


        <h1 class="text-center text-3xl text-green-200 py-4">{{  ip }}</h1>
      ...
      <div class="grid grid-cols-1 gap-4 py-4">
        <div class="py-4  border-1 border-purple-400 shadow-2xl rounded-lg dark:bg-gray-800">
          ...
          <div v-if="history">
            <Association type="ip"  :search_id="this.search_id" :history="this.history"/>
          </div>

          <div v-else>
            <Association type="ip" :keyword="this.ip" :search_id="this.s_id"/>
          </div>
...
export default {
  ...
  components : {Map, Association,Preloader},

  props: ['ip', 'history', 'search_id'],
  ...
  async fetch() {
    ...
    if (!this.history) {
      ...
      let response =  await this.$axios.post("/api/v1/ip/search", {ip: this.ip})
